...
if spin == 2:
    ...
    with open("dos_up.txt", "w") as f:
        for item in dosauxup:
            string = item.to_string(header =True, index=True)
            f.write(f"{string}\n")
    with open("dos_dn.txt", "w") as f:
...
# plot2.set_title("b) $\mathdefault{Ti_3C_2}$",x=-0.3,fontdict={'fontsize': 26})
# plot2.set_xlim(xmin=1,xmax=8)
# plot2.set_ylim(ymin=-75,ymax=125)
# plot2.set_ylabel("J / meV")
# plot2.set_yticks(range(-75,170,50))
